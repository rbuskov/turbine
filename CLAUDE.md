...
## How we work
- Tests-first: Red, green, refactor. Every task in a plan specifies its tests. Implementation without the test is incomplete.
...
- Tests that require mocking more than two collaborators are a smell — the design is probably wrong. Surface this in /review, don't paper over it.
- Coverage is a lagging indicator, not a goal. Target: meaningful assertions, not line counts.

## Architecture
- See `docs/Architecture.md` for the full picture. Always read it before planning a feature that touches multiple layers.
- Current architectural decisions live in `docs/adr/`. Read the index before proposing changes that might contradict one.
...
Do NOT write an ADR for: library choices inside a module, formatting, naming conventions, anything you'd change on a whim.

ADR format: `docs/adr/NNN-short-slug.md`, using the template at `docs/adr/000-template.md`. Status is Proposed → Accepted → Superseded. Never edit an Accepted ADR; supersede it with a new one that links back.
...
- Dont implement things found in Backlog.md (they will be handled later).
- Don't silently change public interfaces. Breaking changes need an ADR or explicit approval.
